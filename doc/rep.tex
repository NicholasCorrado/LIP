...
\abstract{In query optimization on star schemas, lookahead information passing (LIP) is a strategy exploiting the efficiency of probing succinct filters to eliminate almost all facts that do not participate in the final join results. Assuming data independency across all columns in the fact table, it achieves efficient and robust query optimization. This project presents some variants of LIP that can achieve empirically efficient query execution on fact table with correlated data columns, experimented on a skeleton database on top of Apache Arrow.}
